...
%**** Initialize stuff ***
[num_features,num_Spikes] = size(dataPts);
numClust        = 0;
bandSq          = bandWidth^2;
initPtInds      = 1:num_Spikes;
...
beenVisitedFlag = zeros(1,num_Spikes,'uint8');              %track if a points been seen already
numInitPts      = num_Spikes;                               %number of points to posibaly use as initilization points
clusterVotes    = zeros(1,num_Spikes,'uint16');             %used to resolve conflicts on cluster membership
...
while numInitPts
    ...
    thisClusterVotes    = zeros(1,num_Spikes,'uint16');         %used to resolve conflicts on cluster membership

    while 1     %loop untill convergence
        
        sqDistToAll = sum((repmat(myMean,1,num_Spikes) - dataPts).^2);    %dist squared from mean to all points still active
